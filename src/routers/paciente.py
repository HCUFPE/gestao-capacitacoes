...
@router.get("", response_model=List[dict], dependencies=[Depends(auth_handler.decode_token)])
...
@router.get("/{codigo}", response_model=dict, dependencies=[Depends(auth_handler.decode_token)])
